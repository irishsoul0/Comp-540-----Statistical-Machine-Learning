...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    d_out, grads['theta1'], grads['theta1_0'] = affine_relu_backward(dout1, cache1)
    
    loss = loss + 0.5*self.reg*((self.params['theta1']**2).sum() + (self.params['theta2']**2).sum() )
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    self.params['theta' + str(len(hidden_dims)+1)] = weight_scale*np.random.randn(hidden_dims[len(hidden_dims)-1], num_classes)
  ...
  def loss(self, X, y=None):
    ...
    ##No Dropout
    #caches = {}
    #out_i, caches[0] = affine_relu_forward(X, self.params['theta1'], self.params['theta1_0'])
    #for i in range(1,self.num_layers-2):
        #out_i, caches[i] = affine_relu_forward(out_i,self.params['theta' + str(i+1)], self.params['theta' + str(i+1) + '_0'])   
    #scores, caches[self.num_layers-2] = affine_forward(out_i, self.params['theta' + str(self.num_layers-1)], self.params['theta' + str(self.num_layers-1) + '_0'])
    
    ## Dropout added
    if self.use_dropout == False:
        self.dropout_param = {'mode': 'train', 'p': 0}
    caches = {}
    out_i,cache = affine_relu_forward(X,self.params['theta1'],self.params['theta1_0'])
    out_i,drop_cache = dropout_forward(out_i, self.dropout_param)
    caches[0] = {'cache':cache, 'drop_cache':drop_cache}

    for i in range(1,self.num_layers-2):
        out_i,cache = affine_relu_forward(out_i,self.params['theta' + str(i+1)],self.params['theta' + str(i+1) + '_0'])    
        out_i,drop_cache = dropout_forward(out_i, self.dropout_param )
        caches[i] = {'cache':cache, 'drop_cache':drop_cache}

    scores,cache = affine_forward(out_i,self.params['theta' + str(self.num_layers-1)],self.params['theta' + str(self.num_layers-1) + '_0'])
    ...
    #loss, dout2 = softmax_loss(out2, y)
    #dout1, grads['theta2'], grads['theta2_0'] = affine_backward(dout2, cache2)
    #_, grads['theta1'], grads['theta1_0'] = affine_relu_backward(dout1, cache1)
    #loss = loss + 0.5*self.reg*((self.params['theta1']**2).sum() + (self.params['theta2']**2).sum() )
    #grads['theta1'] = grads['theta1'] + self.reg*self.params['theta1']
    #grads['theta2'] = grads['theta2'] + self.reg*self.params['theta2']
    ...
    #dout, grads['theta' + str(self.num_layers-1)], grads['theta' + str(self.num_layers-1)+'_0'] = affine_backward(dloss, caches[self.num_layers-2])
    #for i in range(self.num_layers-2,0,-1):
        #loss = loss + 0.5*self.reg*(np.sum(self.params['theta' + str(i)]**2))
        #dout, grads['theta' + str(i)], grads['theta' + str(i) + '_0'] = affine_relu_backward(dout, caches[i-1])
    ...
    loss = loss + 0.5*self.reg*(np.sum(self.params['theta' + str(self.num_layers-1)]**2))
    d_out,grads['theta'+str(self.num_layers-1)],grads['theta' + str(self.num_layers-1) + '_0'] = affine_backward(d_loss,caches[self.num_layers-2]['cache'])
    
    for i in range(self.num_layers-2,0,-1):
        loss = loss + 0.5*self.reg*(np.sum(self.params['theta' + str(i)]**2))
        d_out = dropout_backward(d_out, caches[i-1]['drop_cache'] )
        d_out,grads['theta' + str(i)],grads['theta' + str(i) + '_0'] = affine_relu_backward(d_out,caches[i-1]['cache'])
        grads['theta' + str(i)] = grads['theta' + str(i)] + self.reg*self.params['theta' + str(i)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
