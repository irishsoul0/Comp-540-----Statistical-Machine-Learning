...
class one_vs_allLogisticRegressor:
    ...
    def train(self,X,y,reg):
       ...
       """
        ...
        for k in self.labels:
            ...
            logi_reg = linear_model.LogisticRegression(C = 1.0 / reg, solver = 'lbfgs', fit_intercept = False)
            logi_reg.fit(X, y == k)
            theta_opt[:, k] = logi_reg.coef_
    ...
    def predict(self,X):
        ...
        y_pred = np.argmax(1.0 / (1.0 + np.exp( - np.dot(X, self.theta))), axis = 1)
